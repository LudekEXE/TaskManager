...
{
        public class TaskController(ITaskService taskService, IServiceScopeFactory serviceScopeFactory)
        {
        ...
        public async Task EditTask()
        {
            ...
            var deadlineInput = Console.ReadLine();
            DateTime? deadline = string.IsNullOrEmpty(deadlineInput) ? null : DateTime.Parse(deadlineInput);
            Console.Write("Podaj nowy priorytet (1-5, lub pozostaw pusty, aby nie zmieniać): ");
            var priorityInput = Console.ReadLine();
            int? priority = string.IsNullOrEmpty(priorityInput) ? null : int.Parse(priorityInput);
